	...
	public class AccesoYVistaCasino
	{
		...
		public void EntrarCasino(XElement parametros)
		{
			try
			{
				...
				// Envio la respuesta
				Vista.Vistas.AccesoYVistaCasino.ObtenerInstancia().ResponderEntradaCasino(id_terminal, usuario, "Bienvenido al CasinoOnlie Grupo 05! Esperamos que gastes mucha plata.", true);
			}
			catch (Exception ex)
			{
		...
		public void SalirCasino(XElement parametros)
		{
			try
			{
				...
				// Envio la respuesta
				Vista.Vistas.AccesoYVistaCasino.ObtenerInstancia().ResponderSalidaCasino(id_terminal, usuario, "Esperamos que la proximas traigas mas plata para gastar! Chau!", true);
			}
			catch (Exception ex)
			{
		...
		public void PedirEstadoCasino(XElement parametros)
		{
			try
			{
				...
				// Envio la respuesta
				Vista.Vistas.AccesoYVistaCasino.ObtenerInstancia().ResponderEstadoCasino(id_terminal, usuario);
			}
			catch (Exception ex)
			{
